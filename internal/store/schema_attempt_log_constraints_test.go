...
func TestAttemptLogSchemaMissingConstraints(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "limiter snapshots accepted on selection_skip",
			query: `
				INSERT INTO attempt_log (
					record_id, logical_request_id, sequence_no, selection_no, record_kind,
					requested_alias, base_alias, upstream_model, account_label, attempt_no, is_spill,
					...
					'rec-skip-limiter-ok', 'request-1', 15, 1, 'selection_skip',
					'alias', 'base', 'upstream', 'k2', NULL, 0,
					100, 200, 200, 'selection_skipped', 'not_applicable',
					0, 0, 0, 'rpm_limit', 1
				)
		...
		{
			name: "zero retry_delay_ms accepted on dispatch",
			query: `
				INSERT INTO attempt_log (
					record_id, logical_request_id, attempt_id, sequence_no, selection_no, record_kind,
					requested_alias, base_alias, upstream_model, account_label, attempt_no, is_spill,
					event_at_us, finished_at_us, selection_wait_us, attempt_duration_us, logical_elapsed_us,
					time_to_first_event_us, outcome, retry_disposition, retry_delay_ms, response_committed,
					usage_observation, dropped_header_count
				) VALUES (
					'rec-retry-zero-ok', 'request-1', 'attempt-1', 16, 1, 'dispatch',
					'alias', 'base', 'upstream', 'k1', 1, 0,
					100, 200, 50, 100, 200,
					150, 'succeeded', 'final', 0, 1,
					'complete', 0
				)
			`,
		},
	...
	for _, test := range []struct {
		...
		{
			name: "limiter snapshots rejected on selection_failure",
			query: `
				INSERT INTO attempt_log (
					record_id, logical_request_id, sequence_no, selection_no, record_kind,
					requested_alias, base_alias, upstream_model, account_label, attempt_no, is_spill,
					event_at_us, finished_at_us, selection_wait_us, logical_elapsed_us,
					outcome, retry_disposition, response_committed,
					limiter_rpm_used, limiter_in_flight
				) VALUES (
					'rec-failure-limiter-bad', 'request-1', 17, 2, 'selection_failure',
					'alias', 'base', 'upstream', NULL, NULL, 0,
					100, 200, 50, 200,
					'no_account_available', 'final', 0,
					0, 0
		...
		{
			name: "negative retry_delay_ms rejected on dispatch",
			query: `
				INSERT INTO attempt_log (
					record_id, logical_request_id, attempt_id, sequence_no, selection_no, record_kind,
					requested_alias, base_alias, upstream_model, account_label, attempt_no, is_spill,
					...
					'rec-retry-negative-bad', 'request-1', 'attempt-1', 18, 1, 'dispatch',
					'alias', 'base', 'upstream', 'k1', 1, 0,
					100, 200, 50, 100, 200,
					150, 'succeeded', 'final', -1, 1,
					'complete', 0
				)
			`,
		},
...
func TestAttemptLogLimiterSnapshotsConstraintOnDispatchRemainsRejected(t *testing.T) {
	s, err := store.Open(filepath.Join(t.TempDir(), "llmux.db"))
	if err != nil {
		t.Fatalf("store.Open() error = %v", err)
	}
	...
	_, err = s.Writer.ExecContext(ctx, `
		INSERT INTO dispatch_admission (
			attempt_id, logical_request_id, attempt_no, account_label,
			requested_alias, upstream_model, reserved_at_us, limiter_rpm_used, limiter_in_flight
		) VALUES ('attempt-dispatch-limiter', 'request-dispatch-limiter', 1, 'k1', 'alias', 'upstream', 100, 0, 0)
	`)
	if err != nil {
		t.Fatalf("insert dispatch_admission: %v", err)
	}
	...
	_, err = s.Writer.ExecContext(ctx, `
		INSERT INTO attempt_log (
			...
			limiter_rpm_used, limiter_in_flight, dropped_header_count
		) VALUES (
			'rec-dispatch-limiter-bad', 'request-dispatch-limiter', 'attempt-dispatch-limiter', 1, 1, 'dispatch',
			'alias', 'base', 'upstream', 'k1', 1, 0,
			100, 200, 50, 100, 200,
			150, 'succeeded', 'final', 1, 'complete',
			0, 0, 0
		)
	`)
	if err == nil {
		t.Fatal("expected constraint failure for limiter snapshots on dispatch, got nil")
	}
}
